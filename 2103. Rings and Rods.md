...
## Solution
For this problem, we are gonna create a hash to keep count the rod-rings and then check if a rod has all the different rings:
...
class Solution:
    def countPoints(self, rings: str) -> int:
        ...
        while index < len(rings):
            color, rod = rings[index], rings[index + 1]
            rods[rod].append(color)
            index += 2

        for _, rod in rods.items():
...
![[Pasted image 20251110140845.png]]
